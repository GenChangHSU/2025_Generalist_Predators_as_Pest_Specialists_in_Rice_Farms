...
### Predator abundance
predator_abd_2017_clean <- arthropod_abd_2017_raw %>%
  select(Farm_ID = Farm, Stage, Family = Family.ID, Abundance = Count) %>%
  filter(Stage != "Seedling") %>%
  mutate(Family = toupper(Family)) %>%
  filter(Family %in% predator) %>%
  mutate(Source = case_when(Family %in% predator ~ "Predator")) %>%
  group_by(Farm_ID, Stage, Source) %>%
  summarise(Predator_abundance = sum(Abundance)) %>%
  ungroup() %>% 
  mutate(Year = 2017)

predator_abd_2018_clean <- arthropod_abd_2018_raw %>%
  select(Date, Farm_ID = Farm, Family = Family.abbr, Abundance) %>%
  filter(Date != "20180402") %>%
  mutate(Farm_ID = str_remove(Farm_ID, pattern = "-"),
         Family = toupper(Family)) %>%
  ...
  mutate(Stage = case_when(Date == "20180506" ~ "Tillering", 
                           Date == "20180608" ~ "Flowering",
                           Date == "20180629" ~ "Ripening"),
         Source = case_when(Family %in% predator ~ "Predator")) %>%
  group_by(Farm_ID, Stage, Source) %>%
  summarise(Predator_abundance = sum(Abundance)) %>%
  ungroup() %>%
  mutate(Year = 2018)

predator_abd_2019_clean <- arthropod_abd_2019_raw %>%
  select(Date, Farm_ID = Farm, Family = Family.abbr, Abundance) %>%
  mutate(Farm_ID = str_remove(Farm_ID, pattern = "-"),
         Family = toupper(Family)) %>%
  filter(Family %in% predator) %>%
...
predator_abd_clean <- bind_rows(predator_abd_2017_clean, 
                                predator_abd_2018_clean,
                                predator_abd_2019_clean) %>%
  select(Year, Farm_ID, Stage, Predator_abundance) %>%
  mutate(Year = as.character(Year))
...
rice_herb_consmp_all <- model_out_clean %>%
  ...
  left_join(., predator_abd_clean, by = join_by("Year", "Farm_ID", "Stage"))
...
### Include predator abundance at the predictor
beta_out_all_median_weighted_predator_abd <- glmmTMB(Proportion_median ~ Year + Farmtype + 
                                          Stage + `Forest_cover_%` + Rel_abd + Predator_abundance + 
                                          (1|Pair_ID/Farm_ID),
                                        data = rice_herb_consmp_all,
                                        weights = weights_vector_all,
                                        na.action = na.exclude,
...
### Test for global significance
lrtest(beta_out_all_median_weighted_predator_abd)

### Test for factor significance
summary(beta_out_all_median_weighted_predator_abd)
Anova(beta_out_all_median_weighted_predator_abd)
...
### Include predator abundance as the predictor
beta_out_spiders_median_weighted_predator_abd <- glmmTMB(Proportion_median ~ Year + Farmtype + 
                                              Stage + `Forest_cover_%` + Rel_abd + Predator_abundance + 
                                              (1|Pair_ID/Farm_ID),
                                            data = rice_herb_consmp_spiders,
                                            weights = weights_vector_spiders,
...
### Test for global significance
lrtest(beta_out_spiders_median_weighted_predator_abd)

### Test for factor significance
summary(beta_out_spiders_median_weighted_predator_abd)
Anova(beta_out_spiders_median_weighted_predator_abd)
...
### Include predator abundance as the predictor
beta_out_ladybeetles_median_weighted_predator_abd <- glmmTMB(Proportion_median ~ Year + Farmtype + 
                                                  Stage + `Forest_cover_%` + Rel_abd + Predator_abundance + 
                                                  (1|Pair_ID/Farm_ID),
                                                data = rice_herb_consmp_ladybeetles,
                                                weights = weights_vector_ladybeetles,
...
### Test for global significance
lrtest(beta_out_ladybeetles_median_weighted_predator_abd)

### Test for factor significance
summary(beta_out_ladybeetles_median_weighted_predator_abd)
Anova(beta_out_ladybeetles_median_weighted_predator_abd)
